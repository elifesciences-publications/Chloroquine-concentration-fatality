...
prior_params = list(max_effect_prior_mu = 90,
                    max_effect_prior_sd = 10, 
                    min_effect_prior_mu = 150,
                    min_effect_prior_sd = 30,
...
CQ_data = list(N = as.integer(sum(ind_notinf)),
               log10_conc = log10(pooled_data$CQ_uMol[ind_notinf]),
               QRS = pooled_data$QRS[ind_notinf],
               study = as.integer(pooled_data$study==1)[ind_notinf])
...
```{r}
pooled_data = dplyr::filter(pooled_data, study<3)
